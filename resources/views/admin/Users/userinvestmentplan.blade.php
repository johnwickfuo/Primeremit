                                    ...
                                    <tbody>
                                        @foreach ($plans as $plan)
                                            <tr>
                                                ...
                                                <td>
                                                    <a href="{{ route('deleteuserinvestment', $plan->id) }}"
                                                        class="m-1 btn btn-info btn-sm"> Delete Plan</a>
                                                    @if ($plan->active == 'yes')
                                                        <a href="{{ route('markuserinvestment', ['id' => $plan->id, 'status' => 'expired']) }}"
                                                            class="m-1 btn btn-danger btn-sm">Mark as expired</a>
                                                    @else
                                                        <a href="{{ route('markuserinvestment', ['id' => $plan->id, 'status' => 'yes']) }}"
