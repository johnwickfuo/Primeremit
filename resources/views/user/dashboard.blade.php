...
<div x-data="{
    ...
    showSendMoney: false,
    showReceiveBtcModal: false, // New: For BTC Receive modal
    currentTime: '',
    greeting: '',
    currentDate: '',
    balanceVisible: true,
    ...
    // New function to handle copying text to clipboard reliably
    copyToClipboard(textToCopy, buttonElement) {
        ...
        try {
            ...
            // Change icon color to green on success
            if (buttonElement) {
                buttonElement.querySelector('i').classList.add('text-green-500');
                setTimeout(() => {
                    buttonElement.querySelector('i').classList.remove('text-green-500');
                }, 1500); // Revert color after 1.5 seconds
            }
        } catch (err) {
...
">
    ...
    <div class="hidden lg:grid grid-cols-4 gap-4 mb-6">
        <div class="bg-gradient-to-r from-primary-50 to-white rounded-xl p-4 border border-primary-100 flex items-center justify-between">
            <div>
                ...
                <p class="text-lg font-bold text-gray-800">{{ Auth::user()->currency_symbol }}{{ number_format(Auth::user()->account_bal,0, '.', ',') }}</p>
            </div>
        ...
        <div class="bg-gradient-to-r from-green-50 to-white rounded-xl p-4 border border-green-100 flex items-center justify-between">
            <div>
                ...
                <p class="text-lg font-bold text-green-700">{{ Auth::user()->currency_symbol }}{{ number_format($monthly_deposits ?? 0,0, '.', ',') }}</p>
            </div>
        ...
        <div class="bg-gradient-to-r from-red-50 to-white rounded-xl p-4 border border-red-100 flex items-center justify-between">
            <div>
                ...
                <p class="text-lg font-bold text-red-700">{{ Auth::user()->currency_symbol }}{{ number_format($monthly_expenses ?? 0,0, '.', ',') }}</p>
            </div>
        ...
        <div class="bg-gradient-to-r from-purple-50 to-white rounded-xl p-4 border border-purple-100 flex items-center justify-between">
            <div>
                ...
                <p class="text-lg font-bold text-purple-700">{{ Auth::user()->currency_symbol }}{{ number_format(Auth::user()->limit, 2, '.', ',') }}</p>
            </div>
    ...
    @if(!empty(Auth::user()->profile_photo_path))
        {{-- CORRECTED: asset() now points to public/ --}}
        <img
            alt="{{ Auth::user()->name }}"
            src="{{ asset('public/'.Auth::user()->profile_photo_path)}}"
        ...
        <div class="font-medium text-white">{{ Auth::user()->name }}</div>
                    ...
                    <div class="mb-6">
                        ...
                        <div x-show="balanceVisible" x-transition class="text-3xl font-bold">
                            {{ Auth::user()->currency_symbol }}{{ number_format(Auth::user()->account_bal,0, '.', ',') }} {{ $settings->s_currency }}
                        </div>
                        <div x-show="!balanceVisible" x-transition class="text-3xl font-bold">
                            *******
                        </div>
                    ...
                    <!-- New: BTC Balance Section -->
                    <div class="mb-6">
                        <div class="flex items-center justify-between">
                            <h2 class="text-lg font-medium mb-1">BTC Balance</h2>
                            <!-- You might want a separate toggle for BTC balance if needed -->
                        </div>
                    ...
                    <!-- New: BTC Action Buttons -->
                    <div class="grid grid-cols-3 gap-4 mb-6">
                        <button
                            @click="showReceiveBtcModal = true"
                            class="flex flex-col items-center justify-center p-3 rounded-xl bg-white/20 hover:bg-white/30 transition-all text-white">
                            <div class="w-10 h-10 rounded-full bg-white/30 flex items-center justify-center mb-2">
                                <i data-lucide="qr-code" class="h-5 w-5 text-white"></i>
                            </div>
                            <span class="text-sm font-medium">Receive</span>
                        </button>

                        <button
                            @click="showSendMoney = true" {{-- Re-use existing send modal, will add BTC option there --}}
                            class="flex flex-col items-center justify-center p-3 rounded-xl bg-white/20 hover:bg-white/30 transition-all text-white">
                            <div class="w-10 h-10 rounded-full bg-white/30 flex items-center justify-center mb-2">
                        ...
                        <a href="{{ route('swap') }}" {{-- Link to a new swap page --}}
                            class="flex flex-col items-center justify-center p-3 rounded-xl bg-white/20 hover:bg-white/30 transition-all text-white">
                            <div class="w-10 h-10 rounded-full bg-white/30 flex items-center justify-center mb-2">
                                <i data-lucide="repeat-2" class="h-5 w-5 text-white"></i>
                            </div>
                            <span class="text-sm font-medium">Swap</span>
                        </a>
                    </div>
                    {{-- End New BTC Action Buttons --}}
...
<div class="bg-white rounded-xl shadow-sm overflow-hidden border border-gray-100 mb-6">
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                ...
                                @forelse($allTransactions as $transaction)
                                    <tr class="hover:bg-gray-50 cursor-pointer" id="{{ $transaction->id }}{{ $transaction->txn_id }}">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="text-sm font-medium text-gray-900">{{ Auth::user()->currency_symbol }}{{ number_format($transaction->amount, 2, '.', ',') }} {{ $settings->s_currency }}</div>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap">
                ...
                <div class="p-6 space-y-4">
                    ...
                    <div class="flex items-center">
                        ...
                        <div class="flex-1 min-w-0">
                            ...
                            <p class="text-lg font-bold text-gray-900 truncate">{{ Auth::user()->currency_symbol }}{{ number_format(Auth::user()->limit, 2, '.', ',') }}</p>
                        </div>
                    </div>
                    ...
                    <div class="flex items-center">
                        ...
                        <div class="flex-1 min-w-0">
                            ...
                            <p class="text-lg font-bold text-gray-900 truncate">{{ Auth::user()->currency_symbol }}{{ number_format($total_deposited_pending + $total_withdrawal_pending, 2, '.', ',') }}</p>
                        </div>
                    </div>
                    ...
                    <div class="flex items-center">
                        ...
                        <div class="flex-1 min-w-0">
                            ...
                            <p class="text-lg font-bold text-gray-900 truncate">{{ Auth::user()->currency_symbol }}{{ number_format($total_withdrawal+$deposited, 2, '.', ',') }}</p>
                        </div>
                    </div>
   ...
   <!-- Bank Account Modal -->
                    ...
                    <ul class="space-y-3">
                        <li class="flex items-center justify-between p-2 hover:bg-gray-100 rounded-lg transition-colors">
                            ...
                            <div class="flex items-center">
                                ...
                                <button class="ml-2 text-primary-500 hover:text-primary-700 focus:outline-none" @click="copyToClipboard('{{ Auth::user()->name }} {{ Auth::user()->lastname }}', $el)">
                        ...
                        <li class="flex items-center justify-between p-2 hover:bg-gray-100 rounded-lg transition-colors">
                            ...
                            <div class="flex items-center">
                                ...
                                <button class="ml-2 text-primary-500 hover:text-primary-700 focus:outline-none" @click="copyToClipboard('{{ Auth::user()->usernumber }}', $el)">
                                    <i data-lucide="copy" class="h-4 w-4 transition-colors duration-300"></i>
                                </button>
                            </div>
                        </li>
                        <li class="flex items-center justify-between p-2 hover:bg-gray-100 rounded-lg transition-colors">
                            ...
                            <div class="flex items-center">
                                ...
                                <button class="ml-2 text-primary-500 hover:text-primary-700 focus:outline-none" @click="copyToClipboard('388130', $el)">
                                    <i data-lucide="copy" class="h-4 w-4 transition-colors duration-300"></i>
                                </button>
                            </div>
                        </li>
                        <li class="flex items-center justify-between p-2 hover:bg-gray-100 rounded-lg transition-colors">
                            ...
                            <div class="flex items-center">
                                ...
                                <button class="ml-2 text-primary-500 hover:text-primary-700 focus:outline-none" @click="copyToClipboard('1234567890', $el)">
                                    <i data-lucide="copy" class="h-4 w-4 transition-colors duration-300"></i>
                                </button>
                            </div>
                        </li>
    ...
    <!-- New: Receive BTC Modal -->
    <div
        x-show="showReceiveBtcModal"
        x-transition:enter="transition ease-out duration-300"
        x-transition:enter-start="opacity-0"
        x-transition:enter-end="opacity-100"
        x-transition:leave="transition ease-in duration-200"
        ...
        x-transition:leave-end="opacity-0"
        class="fixed inset-0 z-50 overflow-y-auto"
        aria-labelledby="receive-btc-title"
        role="dialog"
        aria-modal="true">
        <div class="flex items-center justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
            <div
                ...
                x-transition:leave-end="opacity-0"
                class="fixed inset-0 bg-gray-500 bg-opacity-75 transition-opacity backdrop-blur-sm"
                @click="showReceiveBtcModal = false"
                aria-hidden="true">
            </div>
            ...
            <div
                ...
                <div class="text-center mb-5">
                    <div class="mx-auto flex items-center justify-center h-16 w-16 rounded-full bg-yellow-100 mb-4">
                        <i data-lucide="bitcoin" class="h-8 w-8 text-yellow-600"></i>
                    </div>
                    <h3 class="text-lg font-medium text-gray-900" id="receive-btc-title">Receive Bitcoin (BTC)</h3>
                    <p class="mt-1 text-sm text-gray-500">Send BTC to the address below</p>
                </div>

                <div class="bg-gray-50 p-4 rounded-lg mb-6 text-center">
                    <p class="font-medium mb-3 flex items-center justify-center"><i data-lucide="wallet" class="h-4 w-4 mr-2 text-yellow-500"></i> Your BTC Address</p>
                    <div class="flex items-center justify-center mb-4">
                        {{-- Display QR Code from admin settings --}}
                        @if($settings->btc_qr_code_path)
                            {{-- CORRECTED: asset() now points to public/qr_codes --}}
                            <img src="{{ asset('public/' . $settings->btc_qr_code_path) }}" alt="BTC QR Code" class="w-48 h-48 rounded-md border border-gray-200 p-2">
                        @else
                            <div class="w-48 h-48 bg-gray-200 flex items-center justify-center text-gray-500 rounded-md">No QR Code uploaded</div>
                        @endif
                    </div>
                    <div class="flex items-center justify-center p-2 bg-white rounded-md border border-gray-200">
                        <span id="btcAddressToCopy" class="text-sm font-medium text-gray-800 break-all">{{ $settings->btc_receive_address ?? 'Not set by admin' }}</span>
                        <button class="ml-2 text-primary-500 hover:text-primary-700 focus:outline-none" @click="copyToClipboard('{{ $settings->btc_receive_address ?? '' }}', $el)">
                            <i data-lucide="copy" class="h-4 w-4 transition-colors duration-300"></i>
                        </button>
                    </div>
                </div>
                ...
                <div class="flex items-start p-4 bg-yellow-50 rounded-lg">
                    <i data-lucide="alert-triangle" class="h-5 w-5 text-yellow-500 mt-0.5 mr-3 flex-shrink-0"></i>
                    <p class="text-sm text-gray-700">
        ...
        <div class="flex items-center justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
            ...
            <div
                ...
                <div class="mt-6 space-y-4">
                    ...
                    {{-- New: Send BTC option --}}
                    <a href="{{ route('send-btc') }}" class="block group">
                        <div class="flex items-center justify-between p-4 bg-gray-50 rounded-lg group-hover:bg-gray-100 transition-colors border border-gray-200">
                            <div class="flex items-center">
                                <div class="flex-shrink-0 mr-4">
                                    <div class="h-10 w-10 bg-yellow-100 rounded-full flex items-center justify-center group-hover:bg-yellow-200 transition-colors">
                                        <i data-lucide="bitcoin" class="h-5 w-5 text-yellow-600"></i>
                                    </div>
                                </div>
                                <div>
                                    <h4 class="font-medium text-gray-900">Send Bitcoin (BTC)</h4>
                                    <p class="text-sm text-gray-600">Send BTC to any wallet address</p>
                                    <p class="text-xs text-gray-500">Network fees may apply</p>
                                </div>
                            </div>
                            <div class="w-8 h-8 bg-white rounded-full flex items-center justify-center group-hover:bg-primary-100 transition-colors">
                                <i data-lucide="chevron-right" class="h-5 w-5 text-gray-400 group-hover:text-primary-600 transition-colors"></i>
